...
// --- DATA: INSIGHTS & STATS ---
const insights = [
  { label: "Minimum Age of Scheme", value: "25 Years", icon: <Building2 className="w-6 h-6" /> },
  { label: "Min. Land Area Required", value: "1,500 sqm", icon: <Trees className="w-6 h-6" /> },
  { label: "Free Additional FAR", value: "+1.0", icon: <TrendingUp className="w-6 h-6" /> },
  { label: "Development Charge", value: "50% Off", icon: <Landmark className="w-6 h-6" /> },
  { label: "Stamp Duty (Original Owners)", value: "100% Waived", icon: <FileSignature className="w-6 h-6" /> },
  { label: "Extra Commercial Space", value: "+5%", icon: <Percent className="w-6 h-6" /> },
  { label: "Max Construction Time", value: "3 Years", icon: <Clock className="w-6 h-6" /> },
  { label: "Transit Accommodation", value: "100% Free", icon: <Home className="w-6 h-6" /> }
];

const timelineSteps = [
  { step: 1, title: "AOA Consent", desc: "Minimum 66% (two-thirds) members agree to redevelop.", icon: <Users className="w-5 h-5 text-indigo-600" /> },
  { step: 2, title: "DPR & Approvals", desc: "PMC prepares DPR; Single window clearance by Govt.", icon: <FileSpreadsheet className="w-5 h-5 text-blue-600" /> },
  { step: 3, title: "Vacate Premises", desc: "Residents vacate within 60 days of DPR submission.", icon: <Home className="w-5 h-5 text-amber-600" /> },
  { step: 4, title: "Construction", desc: "Developer builds within 3 years (+2 yrs max extension).", icon: <HardHat className="w-5 h-5 text-orange-600" /> },
  { step: 5, title: "Handover", desc: "New flats allotted (preferably same location/floor).", icon: <CheckCircle2 className="w-5 h-5 text-emerald-600" /> },
];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col md:flex-row justify-between items-center">
          ...
          <nav className="flex flex-wrap gap-1 bg-slate-800 p-1.5 rounded-xl border border-slate-700 shadow-inner">
            {[
              ...
              { id: 'english', label: 'Document (English)' },
              { id: 'hindi', label: 'Document (Hindi)' }
            ].map((tab) => (
              <button
                key={tab.id}
...
function InsightsTab() {
  return (
    <div className="space-y-10 animate-fadeIn">
      ...
      <div className="bg-gradient-to-br from-blue-900 via-indigo-900 to-slate-900 rounded-3xl shadow-2xl p-8 md:p-12 flex flex-col md:flex-row items-center justify-between overflow-hidden relative text-white border border-blue-800">
        <div className="md:w-3/5 z-10">
          ...
          <div className="flex flex-wrap gap-3">
            <span className="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold bg-green-500/20 text-green-300 border border-green-500/40 backdrop-blur-md shadow-lg">
              <CheckCircle2 className="w-4 h-4 mr-2" /> Active Policy
            </span>
            <span className="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold bg-blue-500/20 text-blue-300 border border-blue-500/40 backdrop-blur-md shadow-lg">
              <ShieldCheck className="w-4 h-4 mr-2" /> Single Window Clearance
            </span>
          </div>
        </div>
        <div className="md:w-2/5 mt-12 md:mt-0 flex justify-center z-10">
          <CityAnimation />
        </div>
        {/* Decorative background */}
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-blue-600 rounded-full blur-[150px] opacity-20 pointer-events-none transform translate-x-1/3 -translate-y-1/3"></div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {insights.map((stat, idx) => (
            <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center space-x-5 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
              <div className="p-4 bg-slate-50 text-indigo-600 rounded-xl group-hover:scale-110 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300 shadow-sm border border-slate-100 group-hover:border-indigo-500">
                {stat.icon}
              </div>
              <div>
      ...
      {/* Redevelopment Lifecycle Timeline */}
      <div className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-slate-200">
        <h3 className="text-2xl font-black mb-8 text-slate-800 tracking-tight">The Redevelopment Lifecycle</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8 relative z-10">
            {timelineSteps.map((item, index) => (
              <div key={index} className="flex flex-col items-center text-center group">
                <div className="w-16 h-16 bg-white border-4 border-slate-50 rounded-full flex items-center justify-center shadow-lg mb-4 group-hover:scale-110 group-hover:border-blue-100 transition-all duration-300 relative">
                  {item.icon}
                  <div className="absolute -top-2 -right-2 w-6 h-6 bg-slate-800 text-white text-xs font-bold rounded-full flex items-center justify-center shadow">
                    {item.step}
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Stakeholder Roles */}
        <div className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-slate-200">
          <h3 className="text-2xl font-black mb-8 text-slate-800 flex items-center tracking-tight">
    ...
    <div className="bg-white rounded-3xl shadow-lg border border-slate-200 p-8 md:p-12 animate-fadeIn max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 border-b border-slate-100 pb-8">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">FAR Simulator</h2>
...
function RoleCard({ title, role, items, color, icon }) {
  return (
    <div className={`p-6 rounded-2xl border ${color} bg-opacity-30`}>
      <div className="flex items-center space-x-4 mb-4">
        ...
        <div>
          ...
          <p className="text-xs font-bold uppercase tracking-widest opacity-70">{role}</p>
        </div>
      </div>
      <ul className="space-y-2 mt-4">
        {items.map((item, idx) => (
          <li key={idx} className="flex items-start">
            <div className="w-1.5 h-1.5 rounded-full bg-current mt-2 mr-3 opacity-60"></div>
  ...
  return (
    <div>
      <div className="flex justify-between text-sm mb-2">
        ...
        <span className="font-black text-white bg-slate-800 px-2 py-0.5 rounded-md">{percentage}% Waived</span>
      </div>
